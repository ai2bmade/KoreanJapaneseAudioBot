...
function bottomRankMessage(direction, fromBottom, totalPlayers) {
  ...
  return `今日このセットを受けた ${totalPlayers} 人のうち、あなたの点数は下から ${fromBottom} 番目です。`;
}

function challengeResultMessage(score, total, direction) {
  if (score === total) {
    return direction === "ru_ko" ? ["완벽합니다! 모두 맞혔습니다."] : ["完璧です！全問正解です。"];
  }

  if (score === 0) {
    return direction === "ru_ko" ? ["아이고... 0개 맞혔습니다."] : ["なんてことでしょう……0問正解です。"];
  }
...
function challengeKeyboard() {
  return Markup.inlineKeyboard([
    [Markup.button.callback("Korean -> Japanese", "challenge_ko_ru")],
    [Markup.button.callback("Japanese -> Korean", "challenge_ru_ko")],
    [Markup.button.callback("Next Practice", "practice")]
  ]);
}
...
async function sendChallengeQuestion(ctx, chatId) {
  ...
  const prompt =
    session.direction === "ko_ru"
      ? ["韓国語 -> 日本語", `Задание: ${expression.ko}`, "次の2つのうち、同じ意味のものはどれですか？"]
      : ["일본어 -> 한국어", `문제: ${expression.foreign}`, "다음 둘 중 문제와 같은 뜻은?"];
...
bot.start(async (ctx) => {
  await ctx.reply(
    [
      "こんにちは！",
      "",
      "このボットは、韓国語を学びたい日本語話者のためのリスニング練習ボットです。",
      "同時に、日本語を学ぶ韓国人にも役立つように作りました。",
      "",
      "各音声ファイルには韓国語表現と日本語表現が入り、同じ表現が20回以上繰り返されます。",
      "もう一度聞きたいときは、音声ファイルをもう一度タップしてください。",
      "",
      "無料ユーザーは1日3表現まで聞くことができます。",
      "Buy Me a Coffeeで$5支援していただくと、3か月間、さまざまな表現を制限なく練習できます。",
      "",
      "韓国語のディクテーション練習をしたい方はこちら：",
      "https://t.me/KoreanListeningBot",
      "",
      ...
      "안녕하세요!",
      "",
      "이 봇은 한국어를 배우고 싶은 일본어권 학습자를 위한 듣기 연습 봇입니다. 동시에 일본어를 배우는 한국인에게도 도움이 되도록 만들었습니다.",
      "",
